  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
      <div>
        ...
        <p class="text-slate-500 mt-1" id="emprunt-count">Chargement…</p>
      </div>
      <button onclick="openEmpruntModal()" class="btn-primary flex items-center gap-2 self-start sm:self-auto">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <p class="text-xs text-slate-400 uppercase tracking-wider mb-2">En cours</p>
        <p class="text-3xl font-bold text-amber-500" id="s-encours">—</p>
      </div>
      <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <p class="text-xs text-slate-400 uppercase tracking-wider mb-2">En retard</p>
        <p class="text-3xl font-bold text-red-500" id="s-retard">—</p>
      </div>
      <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <p class="text-xs text-slate-400 uppercase tracking-wider mb-2">Retournés auj.</p>
        <p class="text-3xl font-bold text-green-500" id="s-retours">—</p>
      </div>
      <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <p class="text-xs text-slate-400 uppercase tracking-wider mb-2">Total historique</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-5 flex flex-col sm:flex-row gap-3">
      <input type="text" id="search-input" placeholder="Rechercher adhérent ou document…"
        oninput="filterTable()" class="input-field flex-1">
      <select id="filter-statut" onchange="loadEmprunts()" class="input-field sm:w-44">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              ...
              <th class="text-left px-5 py-3 text-slate-500 font-medium text-xs uppercase tracking-wider">Retour effectif</th>
...
<div id="modal-emprunt" class="fixed inset-0 modal-overlay z-50 hidden flex items-center justify-center p-4">
  <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md">
    <div class="flex items-center justify-between p-6 border-b border-slate-100">
      <h3 class="font-bold text-slate-800 text-lg">Nouveau prêt</h3>
    ...
    <div class="p-6 space-y-4">
      <div>
        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Adhérent</label>
        <select id="em-adherent" class="input-field">
          <option value="">Sélectionner un adhérent…</option>
        </select>
      </div>
      <div>
        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Document</label>
      ...
      <div class="bg-slate-50 rounded-xl px-4 py-3 text-xs text-slate-500 flex items-center gap-2">
        ...
        Retour prévu le <strong class="text-slate-700 ml-1" id="em-date-retour">—</strong>
      </div>
      <div id="em-error" class="hidden bg-red-50 border border-red-200 text-red-600 px-4 py-2.5 rounded-xl text-xs"></div>
    </div>
    <div class="p-6 pt-0 flex gap-3">
      ...
      <button onclick="enregistrerPret()" id="btn-enregistrer" class="flex-1 btn-primary text-center">
        Enregistrer
      </button>
    </div>
  </div>
...
<!-- MODAL RETOUR -->
<div id="modal-retour" class="fixed inset-0 modal-overlay z-50 hidden flex items-center justify-center p-4">
  <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center">
    ...
    <p class="text-slate-500 text-sm mb-7" id="retour-info">Confirmer le retour ?</p>
    <div class="flex gap-3">
      <button onclick="document.getElementById('modal-retour').classList.add('hidden')"
        class="flex-1 px-4 py-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition">
        Annuler
      </button>
...
async function loadEmprunts() {
  ...
  document.getElementById('emprunt-count').textContent =
    `${allEmprunts.length} prêt(s) trouvé(s)`;
}
...
function renderTable(data) {
  ...
  if (!data.length) {
    tbody.innerHTML = `<tr><td colspan="7" class="text-center py-12 text-slate-400">Aucun résultat</td></tr>`;
    return;
  }
  tbody.innerHTML = data.map(e => `
    <tr class="border-t border-slate-50 hover:bg-slate-50/70 transition">
      ...
      <td class="px-5 py-4">
        ...
        <span class="text-xs px-2 py-0.5 rounded-full ${e.type_doc === 'livre' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}">${e.type_doc}</span>
      </td>
      <td class="px-5 py-4 text-slate-500 text-sm">${e.date_emprunt}</td>
      <td class="px-5 py-4 text-sm font-medium ${e.statut === 'en_retard' ? 'text-red-500' : 'text-slate-600'}">${e.date_retour_prevue}</td>
      <td class="px-5 py-4 text-slate-400 text-sm">${e.date_retour_effective || '—'}</td>
      <td class="px-5 py-4">
      ...
      <td class="px-5 py-4">
        ${(e.statut === 'en_cours' || e.statut === 'en_retard') ? `
          <button onclick="initRetour(${e.id}, '${e.titre.replace(/'/g,"\\'")}')"
            class="text-xs font-medium px-3 py-1.5 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition">
            ✓ Retour
...
async function openEmpruntModal() {
  ...
  document.getElementById('em-document').innerHTML =
    ...
    docs.map(d => {
      ...
      return `<option value="${d.code_doc}" data-dispo="${dispo}">${d.titre} (${dispo} dispo.)</option>`;
    }).join('');
...
function checkDispo() {
  ...
  if (!opt || !opt.dataset.dispo) { info.classList.add('hidden'); return; }
  ...
  if (dispo > 0) {
    ...
    info.textContent = '✗ Aucun exemplaire disponible';
  }
}
...
async function enregistrerPret() {
  ...
  if (data.success) {
    ...
    showToast('Prêt enregistré !', 'green');
    loadEmprunts(); loadStats();
  } else {
...
function initRetour(id, titre) {
  retourId = id;
  document.getElementById('retour-info').textContent = `Retour de : "${titre}" ?`;
...
async function confirmerRetour() {
  ...
  const data = await res.json();
  document.getElementById('modal-retour').classList.add('hidden');
  if (data.success) { showToast('Retour enregistré !', 'green'); loadEmprunts(); loadStats(); }
  else showToast('Erreur lors du retour', 'red');
}
